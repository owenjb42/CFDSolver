...
class SolverStaggeredIMEX
{
public:
    SolverStaggeredIMEX(int nx, int ny, double dx, double dy) : nx(nx), ny(ny), dx(dx), dy(dy)
    {
    ...
    void ApplyVelocityBoundaryConditions() 
    {
        ...
        for (int j = 0; j < ny/4; ++j)
        {
            ...
            u(nx, j) = inflowVelocity * 4;
        }

        for (int j = 2 * ny / 4; j < 3 * ny / 4; ++j)
        {
            u(0, j) = inflowVelocity * 3;
            u_coeff(1, j) += coeff;
            u_scr(1, j) += coeff * inflowVelocity * 3;
        }
