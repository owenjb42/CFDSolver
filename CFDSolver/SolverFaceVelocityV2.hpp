...
public:
    ...
    void ComputeConvectiveFluxes()
    {
        ...
        for (int i = 1; i < nx; ++i)
        {
            for (int j = 0; j < ny; ++j)
            {
                if (u_faceFlags(i, j) == 1)
                    continue;

                if (u_faceFlags(i - 1, j) != 1)
                {
                    // Inline Left: u flux
                ...
                if (j != 0 && v_faceFlags(i, j) != 1 && v_faceFlags(i - 1, j) != 1 && u_faceFlags(i, j - 1) != 1)
                {
                    // Bottom Left: u flux
                    flux = (dx / 2.0) * v(i - 1, j);
                    if (flux > 0.0)
                ...
                if (j != ny - 1 && v_faceFlags(i, j + 1) != 1 && v_faceFlags(i - 1, j + 1) != 1 && u_faceFlags(i, j + 1) != 1)
                {
                    // Top Left: u flux
        ...
        for (int i = 0; i < nx; ++i)
        {
            for (int j = 1; j < ny; ++j)
            {
                if (v_faceFlags(i, j) == 1)
                    continue;

                if (v_faceFlags(i, j - 1) != 1)
                {
                    // Inline Left: v flux
                ...
                if (i != 0 && u_faceFlags(i + 1, j) + u_faceFlags(i + 1, j - 1) + v_faceFlags(i + 1, j))
                {
                    // Bottom Left: v flux
                    flux = (dy / 2.0) * u(i, j - 1);
                    if (flux > 0.0)
                ...
                if (i != nx - 1 && u_faceFlags(i, j) + u_faceFlags(i, j - 1) + v_faceFlags(i - 1, j))
                {
                    // Top Left: v flux
    ...
    void SolveVelocitiesForMomentumEquation()
    {
        for (int i = 1; i < nx; ++i)
        {
            ...
            {
                if (u_faceFlags(i, j) != 1)
                    u(i, j) += dt * (u_scr(i, j) + (p(i - 1, j) - p(i, j)) / (fluid.density * dy));
            }
        }
        for (int i = 0; i < nx; ++i)
        {
            ...
            {
                if (v_faceFlags(i, j) != 1)
                    v(i, j) += dt * (v_scr(i, j) + (p(i, j - 1) - p(i, j)) / (fluid.density * dx));
            }
        }
    }
    ...
    void ApplyVelocityBoundaryConditions() 
    {
        ...
        // Friction - ToDo apply from each blocked face
    ...
    PhysicalField u_faceFlags{ nx + 1, ny }; // staggered grid
    PhysicalField v_faceFlags{ nx, ny + 1 }; // staggered grid
